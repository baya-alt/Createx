...
import { MenProductDetails } from "./MenProductDetalis"; 
...
const addToCart = (product, size, color, quantity = 1, imageUrl) => {
  ...
  const item = {
    ...
    quantity: quantity,
    color: color,
    imageUrl: imageUrl || product.avatar || product.avataryellow || product.avatarred || "",
...
function SocialShareModalComponent({ socialType, product, onClose }) {
  ...
  const productImage = product.avatar || product.avataryellow || product.avatarred;
...
/* ================= MAIN MEN PRODUCT PAGE COMPONENT ================= */
export default function MenProductPage() {
  ...
  useEffect(() => {
    fetch(`https://691bbd103aaeed735c8e1d0d.mockapi.io/man/${id}`)
      ...
      .then(data => {
        ...
        const productImages = [
          productData.avataryellow || productData.avatarred || productData.avatar,
          productData.avatar,
          productData.avatarwhite || productData.avatar,
          productData.avatarblue || productData.avatar,
          productData.avataryellow || productData.avatarred || productData.avatar,
          productData.avatar
        ].filter(Boolean);
  ...
  const getColorFromIndex = (idx) => {
    switch(idx) {
      ...
      case 2: return "Blue";
      case 3: 
        if (product?.avataryellow) return "Yellow";
        if (product?.avatarred) return "Red";
        return "Black";
  ...
  const handleAddToCart = () => {
    ...
    const color = getColorFromIndex(realIndex);
    const imageUrl = images[realIndex] || product.avatar;
            ...
            <div className="info-box">
              ...
              <div className="block">
                ...
                <div className="colors">
                  ...
                  {(product.avataryellow || product.avatarred) && (
                    <span 
                      className={`color yellow ${realIndex === 3 ? "active" : ""}`} 
                      onClick={() => handleColorClick(
                        product.avataryellow ? "Yellow" : "Red", 
                        4
                      )} 
                      title={product.avataryellow ? "Yellow color" : "Red color"}
                    />
                  )}
                </div>
                {selectedColor && (
